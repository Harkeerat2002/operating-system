...
thread_get_by_tid (int tid) {
  ...
  for (list  = list_begin(&all_list) ;
       list != list_end  (&all_list) ;
       list  = list_next (list))
  {
    struct thread * elth = list_entry(list, struct thread, allelem);
    if (elth->tid == tid) {
      th = elth;
      break;
    }
...
thread_start (void)
{
  ...
  struct semaphore idle_started; //semafore type
  sema_init (&idle_started, 0);
  thread_create ("idle", PRI_MIN, idle, &idle_started);

  intr_enable (); //preemptive thread scheduling.

  sema_down (&idle_started); //Waits for the idle thread to initialize idle_thread
}
...
void thread_wake_up() {
    ...
    for (pos = list_begin(&all_list); pos != list_end(&all_list);
         pos = list_next(pos)) {
        t = list_entry(pos, struct thread, allelem);

        if (t->sleep_for != 0 && ticks >= t->sleep_for) {
            t->sleep_for = 0;
            thread_unblock(t);
        }
    }
               ...
               thread_func *function, void *aux)
{
  ...
  /* When parent is waiting child,this needs to unblock parent */
 ...
 new_child->tid = tid;
 new_child->status = t->status;
 new_child->exit_code = -1;
 thread_current ()->children;
 list_push_front (&thread_current ()->children, &new_child->elem);
...
#ifdef USERPROG
  first_word_file(t->name, name, 16); /*Max thread lenght = 16*/
#else
  /* In order for the thread tests to continue passing, 
  the thread name must include all words provided to the (const char *name) parameter. */
